...
import { ConnectionNotFoundError, TransactionNotFoundError } from '@/infra/repositories/postgres/errors'
...
describe('PgConnection', () => {
  ...
  it('should get repository from queryRunner', async () => {
    await sut.connect()
    await sut.openTransaction()
    const repository = sut.getRepository(PgTransactionEntity)

    expect(getRepositorySpy).toHaveBeenCalledWith(PgTransactionEntity)
  ...
  it('should return ConnectionNotFoundError on getRepository if connection is not found', async () => {
    expect(() => sut.getRepository(PgTransactionEntity)).toThrow(new ConnectionNotFoundError())
  })
})
